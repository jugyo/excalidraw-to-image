...
- `L4-resilience`: 異常値を含んでも変換継続できる成功ケース

失敗ケースはレベル別 fixture と分離し、`tests/fixtures/failure/` で管理する。
...
- 例:
  ...
  - `l4-resilience-number-fallback.json`
...
## failure fixtures（失敗系）

失敗を期待するケースは `tests/fixtures/failure/` に分離する。
...
2. SVG/PNG ゴールデン生成を実行  
`bun run golden:generate`  
（例: `L1` のみ更新する場合は `bun run golden:generate -- --level l1`）
3. `tests/output/svg/` と `tests/output/png/` の差分を目視確認
